...
Use @MASTG-TECH-0001 to run the following command that will show all schemes:
...
You can use @MASTG-TECH-0001 to test the verification logic regardless of whether the app targets Android 12 (API level 31) or not. This feature allows you to:
...
The following example is a snippet from an example Kotlin app decompiled with @MASTG-TECH-0014. From the [static analysis](#check-for-deep-link-usage) we know that it supports the deep link `deeplinkdemo://load.html/` as part of `com.mstg.deeplinkdemo.WebViewActivity`.
...
Here you will use the list of deep links from the static analysis to iterate and determine each handler method and the processed data, if any. You will first start a @MASTG-TECH-0079 hook and then begin invoking the deep links.
...
Now you can invoke any of the deep links using @MASTG-TECH-0001 and the [Activity Manager (am)](https://developer.android.com/training/app-links/deep-linking#testing-filters "Activity Manager") which will send intents within the Android device. For example:
