...
### Android 16 and accessibilityDataSensitive Protection

Starting with Android 16 (API level 35), developers can use the `accessibilityDataSensitive` flag to provide enhanced protection against malicious accessibility services. This flag addresses partial occlusion scenarios by blocking malicious apps from reading or interacting with sensitive data.

**Key Features:**
- **System-level protection**: Blocks malicious accessibility services from reading sensitive data
- **Automatic protection**: Views using `setFilterTouchesWhenObscured(true)` are automatically treated as accessibility-sensitive
- **Legitimate tool compatibility**: Does not impact legitimate accessibility tools that declare `isAccessibilityTool=true` in their manifest
...
DisposableEffect(Unit) {
    ...
    onDispose { composeView.filterTouchesWhenObscured = false }
}
```

**Testing Recommendations:**
- Verify that sensitive views (login screens, payment flows) use `accessibilityDataSensitive=true`
- Check if the app targets Android 16 (API level 35) or higher
- Test with malicious accessibility services to confirm protection
- Ensure legitimate accessibility tools still function properly
