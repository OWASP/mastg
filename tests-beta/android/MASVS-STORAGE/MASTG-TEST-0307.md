---
title: Sensitive Data Stored Unencrypted via Serialization
platform: android
id: MASTG-TEST-0307
type: [static, dynamic]
weakness: MASWE-0006
...
This test checks if the app uses serialization mechanisms to store sensitive data without proper encryption and integrity protection. Serialized objects might contain sensitive information that could be exposed if not properly protected against eavesdropping or unauthorized modification.
...
- In the AndroidManifest, there will be `meta-data` entries with values such as `DATABASE`, `VERSION`, `QUERY_LOG` and `DOMAIN_PACKAGE_NAME`. Make sure that `QUERY_LOG` is set to false.
...
- If sensitive data is found in serialized objects without encryption and integrity protection, the test fails.
- If serialized data is properly encrypted and signed, and deserialization includes validation, the test passes.