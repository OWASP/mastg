...
This test verifies whether the app detects and responds to instrumentation and hooking attempts in runtime. For example, if the app does not terminate immediately when the following methods are called:

- Authentication tokens, OAuth tokens, session credentials, stored account passwords could be extracted if [`AccountManager.getPassword()`](https://developer.android.com/reference/kotlin/android/accounts/AccountManager#getpassword), [`AccountManager.getAuthToken()`](https://developer.android.com/reference/kotlin/android/accounts/AccountManager#getauthtoken) are hooked.
