...
App's implementing mandatory updates should use [`AppUpdateType.IMMEDIATE`](https://developer.android.com/reference/com/google/android/play/core/install/model/AppUpdateType#IMMEDIATE) rather than [`AppUpdateType.FLEXIBLE`](https://developer.android.com/reference/com/google/android/play/core/install/model/AppUpdateType#FLEXIBLE). The `IMMEDIATE` flow presents a full-screen blocking UI that prevents the user from using the app until the update is fully installed, making it impossible to dismiss or defer. In contrast, `FLEXIBLE` downloads the update in the background and only prompts the user to install, a prompt the user can ignore indefinitely, allowing the app to keep running an outdated and potentially vulnerable version. For security critical updates, only `IMMEDIATE` provides a true enforcement mechanism.
