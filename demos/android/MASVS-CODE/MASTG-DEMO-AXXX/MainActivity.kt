...
fun updateDisplayString(
    ...
    return buildAnnotatedString {
        ...
        try {
            ...
            val demoResults =
                jsonArrayFromString.map { Json.decodeFromJsonElement<DemoResult>(it) }
                ...
                when (demoResult.status) {
                    Status.PASS -> {
                        withStyle(SpanStyle(color = Color.Green)) {
                            append(
                                "MASTG-DEMO-${demoResult.demoId} demonstrated a successful test:\n" +
                                        "${demoResult.message}\n\n"
                            )
                        }
                    }
                    ...
                    Status.FAIL -> {
                        withStyle(SpanStyle(color = Color(0xFFFF9800))) {
                            append(
                                "MASTG-DEMO-${demoResult.demoId} demonstrated a failed test:\n" +
                                        "${demoResult.message}\n\n"
                            )
                        }
                    }
                    ...
                    Status.ERROR -> {
                        withStyle(SpanStyle(color = Color.Red)) {
                            append(
                                "MASTG-DEMO-${demoResult.demoId} failed:\n" +
                                        "${demoResult.message}\n\n"
                            )
                        }
                    }
...
@Composable
fun MainScreen() {
    val defaultMessage = "Click \"Start\" to run the test and launch the vulnerable screen.\n\n"
    var displayString by remember {
        mutableStateOf(buildAnnotatedString { append(defaultMessage) })
    }
