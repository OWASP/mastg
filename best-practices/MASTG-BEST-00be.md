...
Applications that perform business-critical operations, such as financial transactions, multi-factor authentication, or sensitive data handling, should verify the integrity of the device environment before trusting it. Use Android's Key Attestation mechanism (@MASTG-KNOW-01kw) to cryptographically verify that the client's keys reside in hardware-backed storage and that the device has not been compromised.

## Implement Server-Driven Attestation with a Fresh Challenge

Always drive attestation from the server using the challenge-response flow described in @MASTG-KNOW-01kw. Generate a unique, cryptographically random challenge (nonce) for each attestation request using a CSPRNG. Never reuse challenges across requests. Never implement attestation verification solely on the client side.

## Verify the Attestation Certificate Chain

On the server side, verify the attestation certificate chain as described in @MASTG-KNOW-0044:

- Verify the chain of trust up to the [Google Hardware Attestation Root Certificate](https://developer.android.com/training/articles/security-key-attestation#root_certificate).
- Check each certificate against Google's [Certificate Revocation Status List](https://developer.android.com/training/articles/security-key-attestation#certificate_status).
...
- Check the verified boot state, boot key, and bootloader lock status to ensure the device has not been unlocked or tampered with.

## Enforce Key Properties

Use the attestation extension data to confirm that the attested key pair was generated with the expected properties (@MASTG-KNOW-01kw):
